...
$(document).ready(function () {
    ...
    $('body').on('click', 'li.wc_payment_method > label:not([for="payment_method_tpaysf"])', function () {
        $("#carddata").val('');
        $("#card_vendor").val(getCreditCardNameByNumber(''));
        $("#card_short_code").val('');
        $('#card_number').val('');
        $('#expiry_date').val('');
        $('#cvc').val('');
...
function CardPayment(pubkey) {
    ...
    function validateCcNumber($elem) {
        var isValid = false,
            ...
            type = getCreditCardNameByNumber(ccNumber),
        ...
        if (supported.indexOf(type) < 0 && type !== null && ccNumber.length > 1) {
            ...
            setWrong($elem);
        } else if (supported.indexOf(type) > -1 && isCardNumberValid(ccNumber)) {
            setValid($elem);
    ...
    function validateCvc($elem) {
        var isValid = false;
        if (isSecurityCodeValid($elem.val())) {
            setWrong($elem);
        } else {
            setValid($elem);
            isValid = true;
