...
function createInstallmentContainer(amount) {
    let url = `https://secure.tpay.com/Installment/Pekao/page?merchantId=${tpayCart.merchantId}&amount=${amount}`;
    ...
    if (!document.querySelector('.installments-button')) {
        ...
        installmentsButton.classList.add('btn', 'installments-button', 'cart');
        ...
        const text = document.createTextNode(tpayCart.translations.button);
