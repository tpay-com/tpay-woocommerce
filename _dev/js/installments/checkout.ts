...
document.addEventListener('DOMContentLoaded', function () {
    function createInstallmentsButton() {
        if (tpayCheckout.amount >= 100 && tpayCheckout.amount <= 20000) {
            createInstallmentContainer();
        }
    }

    function createInstallmentContainer() {
        let url = `https://secure.tpay.com/Installment/Pekao/page?merchantId=${tpayCheckout.merchantId}&amount=${tpayCheckout.amount}`;
        let summaryTotalElement = document.querySelector('.woocommerce-checkout-review-order-table');
        ...
        if (summaryTotalElement) {
            ...
            installmentsButton.classList.add('btn', 'installments-button', 'checkout');
            ...
            const text = document.createTextNode(tpayCheckout.translations.button);
