...
class TpayTwisto extends TpayGateways
{
    ...
    {
        parent::__construct(TPAYTWISTO_ID, TPAYTWISTO);
        $this->has_terms_checkbox = true;
        $this->icon = apply_filters('woocommerce_tpay_icon', plugin_dir_url(__FILE__).'../views/img/twisto.png');
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if ('success' == $result['result']) {
            ...
            $order->save();
            $this->gateway_helper->tpay_logger('Udane zamówienie, płatność Twisto, redirect na: '.$redirect);
