...
class ConfigProvider
{
    public function get_config(TpayGateways $gateway)
    {
        ...
        if ($gateway->get_option('use_global') === 'yes' || !$gateway->get_option('use_global')) {
            $result['security_code'] = $gateway->get_tpay_option(['tpay_settings_option_name', 'global_security_code']);
            $result['api_key'] = $gateway->get_tpay_option(['tpay_settings_option_name', 'global_api_key']);
            $result['api_key_password'] = $gateway->get_tpay_option(['tpay_settings_option_name', 'global_api_key_password']);
        } else {
            if ($gateway->id != TPAYSF_ID) {
                $result['security_code'] = $gateway->get_option('security_code');
                ...
                if ($gateway->valid_mid) {
                    $result['security_code'] = $gateway->get_option('security_code' . $gateway->valid_mid);
