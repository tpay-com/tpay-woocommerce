...
class Transactions
{
    ...
    public function __construct(Client $client, Cache $cache, TpayLogger $logger)
    {
        if ($client->connect() === false) {
            throw new \RuntimeException('Could not connect to Open API');
        }

        $this->client = $client->connect()->transactions();
        $this->cache = $cache;
        $this->logger = $logger;
    }
